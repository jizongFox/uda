...
def save_tfrecord(example_list, out_path, max_shard_size=4096):
    ...
    record_writer.close()
    tf.logging.info("saved {} examples to {}".format(
        len(example_list), out_path))
    ...
    if FLAGS.task_name == "cifar10":
        ...
        for split in ["train", "test"]:
            ...
            filename = os.path.join(
                download_folder, "{}_32x32.mat".format(split))
...
def proc_and_dump_sup_data(sub_set_data, split, sup_size=-1):
    ...
    if split == "train" and FLAGS.dev_size != -1:
        dev_images, dev_labels, train_images, train_labels = get_data_by_size_lim(
            ...
            return_rest=True)
        process_and_save_sup_data(
            train_images, train_labels, "train", train_images.shape[0])
        process_and_save_sup_data(
            dev_images, dev_labels, "dev", dev_images.shape[0])
    else:
